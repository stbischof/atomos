...
public final class ModuleConnectLoader extends SecureClassLoader
		implements
			BundleReference {
	...
	public ModuleConnectLoader(ResolvedModule resolvedModule,
			AtomosRuntimeModules atomosRuntimeModules) throws IOException {
	 ...
	 *
	 * @param module
	 *            module associated with this class loader
	 * @param loaderConfig
	...
	void initEdges(Module module, Configuration loaderConfig,
		...
		for (ResolvedModule moduleRead : resolvedModule.reads()) {
			...
			if (moduleRead.configuration().equals(loaderConfig)) {
				...
				ClassLoader cl = module.getLayer()
						.findLoader(moduleRead.name());
				loaderForModuleRead = (cl != null)
						? cl
						: ClassLoader.getPlatformClassLoader();
			}
			moduleRead.reference().descriptor().exports()
					.forEach(packageExport -> {
						if (!packageExport.isQualified() || packageExport
								.targets().contains(module.getName())) {
							edges.putIfAbsent(packageExport.source(),
									loaderForModuleRead);
						}
					});
		}
	...
	/**
	 * Returns a URL to a resource of the given name in a module defined to this
	 * class loader.
	 */
	@Override
	protected URL findResource(String moduleName, String name)
		...
		if (!this.resolvedModule.name().equals(moduleName)) {
			...
			try {
				resource = AccessController
						.doPrivileged((PrivilegedExceptionAction<URL>) (() -> {
	...
	public URL findResource(String name) {
		...
		if (this.module.get().getDescriptor().packages().contains(pkg)) {
			// non-class resources should be returned by this method if the
			// package is open unconditionally
	...
	public Enumeration<URL> getResources(String name) throws IOException {
		// first check this class loader resources same as findResources(String
		// name) does
	...
	protected Class<?> findClass(String className)
		...
		// use SecureClassLoader.defineClass(String, ByteBuffer, CodeSource) to
		// define the class
		// any unexpected checked exceptions should be wrapped in a LinkageError
		// that LinkageError should be thrown
		Class<?> cls = null;
		ByteBuffer clsBytes = null;
		try {
			Optional<ByteBuffer> optBB = this.reader
					.read(className.replace('.', '/') + ".class");
			if (optBB.isPresent()) {
				clsBytes = optBB.get();
	...
	protected Class<?> loadClass(String className, boolean resolve)
		...
		synchronized (getClassLoadingLock(className)) {
			...
			if (cls == null) {
				// otherwise; check ModuleDescriptor.pacakges to see if it
				// contains the package for the requested class
				...
				if (module.get().getDescriptor().packages().contains(pkg)) {
					...
					// otherwise; check for packages this module can read from
					// and if you have another loader for the package
			...
			// if any class is found then call resolveClass on it if the resolve
			// param is true
			else if (resolve) {
				resolveClass(cls);
			}
	...
	protected PermissionCollection getPermissions(CodeSource cs) {
		...
		// This collection may need another permission added to it if the code
		// source location represents
		// a directory. In this case a recursive FilePermission needs to be
		// added to the collection that
